...
import com.example.mloong.yidonghulian.adapter.GoodsListAdapter;
import com.example.mloong.yidonghulian.adapter.OnGoodsItemClickListener;
...
import com.example.mloong.yidonghulian.presenter.GoodsPresenter;
...
public class GoodsListActivity extends BaseActivity {
    ...
    private void initViews() {
        ...
        adapter.setOnGoodsItemClickListener(new OnGoodsItemClickListener() {
            @Override
            public void onClick(View view, Goods entity) {
    ...
    private void loadGoodsListByCatId(int catId) {
        GoodsPresenter.list(new ProgressDialogSubscriber2<HttpResult<List<Goods>>>(this) {
            @Override
            public void onNext(HttpResult<List<Goods>> listHttpResult) {
                List<Goods> goodsEntities = listHttpResult.getData();
                if (listHttpResult.getStatus().equals(Share.SUCCESS)) {
                    ...
                    Toast.makeText(GoodsListActivity.this, "加载二级分类id查询商品列表失败", Toast.LENGTH_SHORT).show();
                }
            }
        }, catId);
    }
    ...
    private void loadGoodsListByKeywords(String keyword) {
        GoodsPresenter.listByKeywords(new ProgressDialogSubscriber2<HttpResult<List<Goods>>>(this) {
            @Override
            public void onNext(HttpResult<List<Goods>> listHttpResult) {
                if (listHttpResult.getStatus().equals(Share.SUCCESS)) {
                    List<Goods> goodsEntities = listHttpResult.getData();
                    ...
                    Toast.makeText(GoodsListActivity.this, "根据搜索关键词来搜索商品列表失败", Toast.LENGTH_SHORT).show();
                }
            }
        }, keyword);
    ...
    @OnClick({R.id.title_back, R.id.search_keyword, R.id.goodslist_orderby_sales,
            R.id.goodslist_orderby_grade, R.id.goodslist_orderby_price, R.id.goodslist_orderby_newgoods})
