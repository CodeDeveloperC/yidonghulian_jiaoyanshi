...
public class Ad2Activity extends BaseActivity {
    ...
    private static class MyHandler extends Handler {
        private final WeakReference<Ad2Activity> mActivity;

        public MyHandler(Ad2Activity activity) {
            mActivity = new WeakReference<>(activity);
        }

        @Override
        public void handleMessage(Message msg) {
            Ad2Activity activity = mActivity.get();
            if (activity != null) {
                if (msg.what == -1) {
//                    skip();
                    activity.flag++;
    ...
    private void skip() {
        if (flag == 1) {
            startActivity(new Intent(Ad2Activity.this, MainActivity.class));
            finish();
        }
    }
